...
├── data/                          # 数据层 (Data Layer)
│   ├── models/                    # 数据模型/实体
│   ├── repositories/              # 仓库模式（数据访问抽象）
│   ├── data_sources/              # 数据源
│   │   ├── remote/                # 远程数据源（API）
│   │   └── local/                 # 本地数据源（数据库/缓存）
│   └── services/                  # 服务层（业务服务）
│
├── domain/                        # 领域层 (可选，Clean Architecture)
│   ├── entities/                  # 领域实体
│   ├── repositories/              # 仓库接口
│   └── use_cases/                 # 用例/业务逻辑
│
...
数据层（Data Layer / Repository） - 数据访问和管理
作用： 负责获取、保存、更新数据，与 API/数据库交互
位置： lib/providers/ (你项目中的数据 Provider)
特点：
✅ 直接与 API Service 交互
✅ 管理数据的 CRUD 操作（创建、读取、更新、删除）
✅ 缓存和持久化数据
✅ 对外提供数据访问接口
❌ 不包含 UI 逻辑
❌ 不处理复杂的业务组合逻辑
...
视图模型层（ViewModel Layer） - UI 业务逻辑
作用： 为特定页面/功能提供 UI 状态和业务逻辑，组合多个数据源
位置： lib/riverpod/ (你项目中的 ViewModel)
特点：
✅ 管理 UI 状态（加载状态、错误信息、用户交互状态）
✅ 组合多个数据源（多个 Provider、Service）
✅ 处理复杂的业务逻辑（动画、轮询、状态协调）
✅ 为特定页面/功能服务
✅ 将业务逻辑从 Widget 中分离
❌ 不直接调用 API（通过数据层调用）
...
└─────────────────────────────────────────────────────────────┘
            ↑                    ↑                    ↑
            │                    │                    │
     监听 PetProvider      监听 EventProvider    使用 Service
            │                    │                    │
┌───────────┴────────────────────┴────────────────────┴───────┐
│                      Data Layer                             │
│                   (lib/providers/)                          │
│  ┌─────────────┐  ┌─────────────┐  ┌──────────────────┐   │
│  │PetNotifier  │  │EventNotifier│  │BluetoothManager  │   │
│  │- loadPets() │  │- loadEvents │  │- connect()       │   │
│  │- createPet()│  │- createEvent│  │- readData()      │   │
│  └─────────────┘  └─────────────┘  └──────────────────┘   │
└─────────────────────────────────────────────────────────────┘
            ↑                    ↑
            │                    │
...
| 容器类型 | 性能 | 使用场景 | 注意事项 |
|---------|------|---------|---------|
| `Container` | 中 | 需要多种样式属性时 | 功能多但性能略低 |
| `SizedBox` | 高 | 固定尺寸、间距 | 单一职责，性能最优 |
| `Padding` | 高 | 纯内边距 | 优于 Container |
| `DecoratedBox` | 高 | 纯装饰 | 优于 Container |
| `ListView` | 高 | 长列表 | 懒加载优化 |
| `SingleChildScrollView` | 中 | 短内容滚动 | 全部渲染，慎用长列表 |
...
| 场景 | 选择 | 原因 |
|------|------|------|
| 展示静态数据 | StatelessWidget | 性能更好，代码更简洁 |
| 需要用户交互 | StatefulWidget | 需要响应状态变化 |
| 动画效果 | StatefulWidget | 需要管理动画控制器 |
| 表单输入 | StatefulWidget | 需要管理输入状态 |
| 从父组件接收数据展示 | StatelessWidget | 数据由父组件管理 |
| 需要定时更新 | StatefulWidget | 需要管理定时器状态 |
...
### 1. RenderFlex overflow（布局溢出）
```dart
// ❌ 错误
Column(
  children: [
    Text('Very long text...'), // 可能溢出
  ],
)
...
// ✅ 解决方案 3：使用 SingleChildScrollView
SingleChildScrollView(
  child: Column(
    children: [
      Text('Very long text...'),
...
### 创建新页面时
- [ ] 确定是 StatelessWidget 还是 StatefulWidget
- [ ] 添加 Scaffold 作为页面骨架
- [ ] 配置 AppBar
- [ ] 处理生命周期（如果是 StatefulWidget）
- [ ] 清理资源（dispose 方法）
...
### 布局时
- [ ] 避免无限约束冲突
- [ ] 使用 Expanded/Flexible 处理弹性布局
- [ ] 测试不同屏幕尺寸
- [ ] 添加 SingleChildScrollView（如果内容可能溢出）

### 发布前
- [ ] 移除所有 print/debugPrint
- [ ] 测试不同设备
- [ ] 检查权限配置
- [ ] 更新版本号
...
### 1. **FlutterBoost** - 混合开发
- **适用**：Flutter 与原生（iOS/Android）混合开发
- **场景**：已有原生 App，逐步迁移到 Flutter
...
dependencies:
  ...
  # 根据需求选择
  fl_chart: ^0.65.0  # 需要图表时
  flutter_slidable: ^3.0.0  # 需要侧滑时
  photo_view: ^0.14.0  # 需要图片查看时
```
...
class MyText extends StatelessWidget {
  ...
  // 2. 构造函数（使用 const 提升性能）
  const MyText({
    super.key,
    required this.text,  // required 表示必传参数
  });
  
  // 3. build 方法（返回 Widget）
  @override
  Widget build(BuildContext context) {
    return Text(text);
  }
...
class _CounterState extends State<Counter> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        Text(widget.title),  // 访问 Widget 属性
        Text('Count: $_count'),
        ElevatedButton(
          onPressed: _increment,
          child: Text('+'),
...
// 使用
Counter(title: 'My Counter', initialValue: 10)
```
...
class CustomButton extends StatelessWidget {
  ...
  final VoidCallback? onPressed;  // 点击回调（可选）
  final Color? backgroundColor;   // 背景色（可选）
  final Color? textColor;         // 文字颜色（可选）
  ...
  const CustomButton({
    super.key,
    required this.text,           // 必传
    this.onPressed,               // 可选
    this.backgroundColor,         // 可选
    this.textColor,               // 可选
    this.width,                   // 可选
    this.height,                  // 可选
  });
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: width ?? 200,        // 默认宽度 200
      height: height ?? 48,       // 默认高度 48
      child: ElevatedButton(
        onPressed: onPressed,
        style: ElevatedButton.styleFrom(
          backgroundColor: backgroundColor ?? Colors.blue,  // 默认蓝色
          foregroundColor: textColor ?? Colors.white,       // 默认白色
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(8),
          ),
        ),
...
class CustomTextField extends StatelessWidget {
  ...
  final String? hint;                    // 占位符
  final TextEditingController? controller;  // 控制器
  final TextInputType? keyboardType;     // 键盘类型
  final bool obscureText;                // 是否隐藏文本（密码）
  final IconData? prefixIcon;            // 前缀图标
  final IconData? suffixIcon;            // 后缀图标
  final VoidCallback? onSuffixIconTap;   // 后缀图标点击
...
class CustomCard extends StatelessWidget {
  ...
  final Widget? trailing;                // 右侧组件
  final VoidCallback? onTap;             // 点击回调
  final List<Widget>? actions;           // 底部操作按钮
...
class ExpandableListItem extends StatefulWidget {
  final String title;                    // 标题
  final String? subtitle;                // 副标题
  final IconData? leadingIcon;           // 前导图标
  final Widget child;                    // 展开后显示的内容
  final bool initiallyExpanded;          // 初始是否展开
  final ValueChanged<bool>? onExpansionChanged;  // 展开状态改变回调
...
class MyComponent extends StatelessWidget {
  // ✅ 好的设计
  final String title;              // 必传参数
  final String? subtitle;          // 可选参数（用 ?）
  final VoidCallback? onTap;       // 回调函数
  final Color backgroundColor;     // 有默认值
  
  const MyComponent({
    super.key,
    required this.title,           // required 标记必传
    this.subtitle,                 // 可选
    this.onTap,                    // 可选
    this.backgroundColor = Colors.blue,  // 默认值
  });
...
class MyText extends StatelessWidget {
  ...
  const MyText({super.key, required this.text});  // const 构造函数
...
class CustomButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      height: height ?? defaultHeight,  // 使用常量
      child: ElevatedButton(
        style: ElevatedButton.styleFrom(
...
class ComplexComponent extends StatelessWidget {
  ...
  const ComplexComponent({super.key, required this.items});
...
class ThemedButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ElevatedButton(
      onPressed: onPressed,
      style: ElevatedButton.styleFrom(
        backgroundColor: theme.primaryColor,      // 主题色
        foregroundColor: theme.colorScheme.onPrimary,
      ),
      child: Text(
        text,
        style: theme.textTheme.labelLarge,       // 主题文字样式
      ),
    );
  }
}
...
### 开发自定义组件时
- [ ] 选择 StatelessWidget 还是 StatefulWidget
- [ ] 定义必要的属性（final）
- [ ] 使用 const 构造函数
- [ ] 提供合理的默认值
- [ ] 添加文档注释
- [ ] 处理边界情况（null、空值等）
- [ ] 考虑性能优化（const、RepaintBoundary）
- [ ] 提取可复用的部分
...
1. **选择合适的基类**：StatelessWidget vs StatefulWidget
2. **设计良好的 API**：清晰的属性、合理的默认值
3. **提供灵活性**：可选参数、回调函数
4. **注重性能**：使用 const、避免不必要的重建
5. **可维护性**：文档注释、命名规范、代码分离
6. **可复用性**：提取通用逻辑、支持主题化

**记住**：一个好的自定义组件应该是**简单易用、功能明确、高度可复用**的！🎯
