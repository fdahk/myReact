...
8.27：
    ...
    2.迭代器（Iterator）是一种设计模式，它提供了一种统一的方式来遍历不同类型的数据结构。在 JS 中，迭代器是一个对象，它实现了迭代器协议。
        ...
        状态保持：迭代器会保持当前的遍历状态
        性能考虑：迭代器通常比“一次性先生成完整结果数组”更节省整体内存，但这不代表它一定更快；是否更快还取决于引擎优化、是否有解构、回调调用、临时对象分配等因素
...
## 为什么 `for`、`for...of + entries()`、`slice().forEach()` 的性能会不一样

先看这三种写法的本质。它们做的事情完全一样，都是在给动态规划做“边界初始化”：
...
for (const [i, row] of grid.entries()) {
    if (i > 0) row[0] += grid[i - 1][0];
}
for (const [j] of grid[0].entries()) {
...
它们的**算法复杂度**没有变，都是 `O(m + n)`。差别主要不在算法层，而在**执行路径的额外成本**。

### 一、先说结论

你看到的差异，既有**代码本身确实存在的底层区别**，也有**运行环境和测试方式带来的波动**。

可以把它理解成：

- `for`：最接近底层，额外动作最少，通常最容易被 JS 引擎优化
- `for...of + entries()`：语义更强，但要走迭代器协议，还伴随解构和临时值处理，通常更重
- `slice().forEach()`：有额外的浅拷贝和回调调用成本，但有时比 `entries()` 更容易跑得不差，甚至在某些测试里更快

所以不是“纯环境问题”，也不是“百分百固定谁更快”。更准确地说是：

> 写法本身有真实的底层成本差异，而在线评测环境又会把这种差异放大、缩小，甚至随机化。
...
它几乎没有额外抽象层，所以 JS 引擎最容易把它优化成“稳定、可预测”的机器码路径。

#### 2. `for...of + entries()` 在做什么
...
1. `grid.entries()` 先返回一个**数组迭代器对象**
2. `for...of` 每次循环都要调用迭代器的 `next()`
3. `entries()` 返回的是 `[index, value]`
4. `const [i, row]` 会对这个返回值做**解构赋值**

也就是说，你表面上写的是“优雅遍历”，底层实际上走的是：
...
- **迭代器（Iterator）**：一种按顺序产出数据的对象，核心是 `next()`
- **迭代协议**：对象实现 `next()`，每次返回 `{ value, done }`
- **可迭代对象（Iterable）**：实现了 `Symbol.iterator`，可被 `for...of` 遍历
- **解构赋值**：把数组或对象拆开取出字段，例如 `[i, row]`

这些机制让代码更通用，但每一步都可能带来额外成本。

尤其是 `entries()` 的 `value` 是 `[index, element]` 这种二元结构，规范层面它并不是“白送”的。

#### 3. `slice().forEach()` 在做什么
...
1. `slice(1)` 会创建一个**新数组**
2. `forEach()` 会对每个元素调用一次回调函数

但这里要注意，`slice(1)` 对二维数组做的是**浅拷贝**：

- 新数组会被创建
- 复制的是每一行数组的**引用**
- 并不会深拷贝每个 `row`

所以它不是把整个二维数组都复制了一遍，而是复制了一份“行引用列表”。

这也是为什么它可能没有你想象中那么重。

### 三、为什么 `entries()` 往往比传统 `for` 慢

核心原因是：**抽象层越多，执行时参与的机制就越多**。
...
`for...of` 不是简单的“语法糖替换成普通下标循环”。

它必须遵守 JS 的统一迭代协议，所以引擎要按协议拉取数据。协议的好处是统一、通用，代价是比直接数组下标访问更难做到极致轻量。

#### 2. 解构成本

`const [i, row] = ...` 看起来简洁，但解构本身也要做值读取和绑定。

如果循环次数很多，这种细小成本会累计。

#### 3. 临时对象或中间值成本

从语言规范角度看，`entries()` 每步产出的结果不是“裸索引 + 裸值”两个寄存器概念，而是要形成可被协议消费的结果。

不同引擎会做不同程度优化，但整体上它依旧比直接 `grid[i]` 更重。

#### 4. 更难被优化到最极致

JS 引擎喜欢“形状稳定、类型稳定、控制流简单”的代码。

传统 `for` 非常符合这一点，而迭代器 + 解构 + 泛型协议会让优化器要考虑更多分支和通用性。

### 四、为什么 `slice().forEach()` 有时会比 `entries()` 快

这也是很多人第一次看到结果时最困惑的点。

表面上看，`slice()` 明明创建了新数组，`forEach()` 明明也要回调，为什么还能比 `entries()` 快？

原因通常有这几个：

#### 1. `slice(1)` 只做浅拷贝

它复制的是一层引用，不是深拷贝二维数组全部内容，所以实际成本可能并不高。

#### 2. `forEach()` 不需要每轮产出 `[index, value]`

`entries()` 的模型是“每一步返回一对键值”，而 `forEach()` 的模型是“引擎按顺序把元素喂给回调”。

在某些引擎实现里，后者的内部路径可能更直接。

#### 3. 内建方法可能有专门优化

像 `forEach()`、`slice()` 这类数组内建方法，JS 引擎往往会有比较成熟的优化路径。

这不代表它一定比 `for` 快，而是说它未必一定输给 `entries()`。

#### 4. 你的测试规模可能很小

如果 `m` 和 `n` 不大，那么一次 `slice()` 的成本可能几乎可以忽略，但 `entries()` 每轮的协议成本却会持续存在。

所以在小中规模数据上，`slice().forEach()` 完全可能跑出比 `entries()` 更好的结果。

### 五、为什么在线评测里会出现 `0ms`、`3ms`、`14ms`
...
它可能只跑少量样例，或只统计一次主要执行时间。只要测试次数不够，波动就会很大。

#### 3. JIT 预热问题

现代 JS 引擎（如 V8）不是一上来就直接用最优机器码执行。
...
这叫做**JIT（Just-In-Time Compilation，及时编译）**。

如果你的代码在“还没充分热起来”时就被计时，结果和热身后的表现可能完全不同。

#### 4. 垃圾回收（GC）干扰

如果某种写法产生了更多临时对象、闭包或中间结果，就更可能触发或接近垃圾回收。

GC 什么时候发生，并不完全由你这几行代码决定，还跟当时运行环境的内存状态有关。
...
同一时刻机器负载、CPU 频率、系统调度、容器状态都可能影响毫秒级结果。

### 六、本质上是在和什么“对抗”

本质上，你不是在和“算法复杂度”对抗，而是在和**语言抽象成本**对抗。

也可以这样理解：

- 算法层：三种写法都在做同一件事，复杂度差不多
- 运行时层：谁创建的中间对象更少、调用层级更少、类型更稳定，谁更容易快
- 引擎层：谁更符合 JIT 的优化模型，谁更可能被编译成高效机器码

所以微观性能优化的本质，不是“写更炫的语法”，而是尽量让代码：
...
`minPathSum` 这道题的主算法是**动态规划（Dynamic Programming, DP）**。

这里的前两段循环是在初始化边界：

- 第一列只能从上往下走，所以每个位置都等于“自己 + 上面累计值”
- 第一行只能从左往右走，所以每个位置都等于“自己 + 左边累计值”
...
所以前面三种不同写法，只是在实现“第一列初始化”和“第一行初始化”。
...
这里三种写法的复杂度都一样，所以它们的性能差异不属于“大 O 级别差异”，而属于**常数项差异**。

#### 2. 常数项

比如两个算法都是 `O(n)`，但一个每轮只做 3 步，另一个每轮做 8 步，那么后者常数项更大。

这正是这三种写法差异的核心。

#### 3. JIT

JS 在运行时边执行边编译热点代码，以获得更高性能。

#### 4. 内联（Inline）

优化器会尝试把一些调用展开，减少函数调用成本。

传统 `for` 更容易形成简单可优化路径；回调和迭代器往往更复杂。

#### 5. 去优化（Deoptimization, deopt）

某段代码原本被乐观优化了，但运行时发现假设不成立，就退回更通用但更慢的路径。

比如类型突然变化、数组结构不稳定，都可能影响优化结果。

### 九、实践中应该怎么选

如果你在做算法题或高频循环，优先级通常是：
...
- 算法题、性能敏感路径：优先传统 `for`
- 日常业务代码：可读性优先，`for...of` 或数组方法都可以
- 真要比较性能：不要看单次在线评测结果，要自己做多轮 benchmark
...
#### 2. 不要把“更现代的语法”理解成“必然更快”

现代语法通常是为了可读性、表达力、抽象能力，不是专门为极限性能设计的。

#### 3. 不要忽略可读性和维护成本

如果循环不在性能热点上，过度手写微优化往往不值得。

#### 4. 真正的性能瓶颈通常先看算法，再看数据结构，最后才看循环语法
...
- V8 的优化与去优化机制
- 数组的稠密数组（packed array）与稀疏数组（holey array）
- 隐藏类（hidden class）与内联缓存（inline cache）
- `for`、`for...of`、`forEach`、`map` 在不同引擎中的优化差异
- 微基准测试为什么容易误导
...
> 它们虽然完成同样的业务逻辑，但走的 JS 运行时机制不同，额外抽象层、临时分配、回调调用、迭代器协议和引擎优化难度都不同，所以会形成真实的常数项差异；而在线评测环境又会让这种差异呈现出波动甚至失真的结果。
...
很多人一提到“JS 数据结构”，脑子里首先想到的是 `Object`、`Array`、`Map`、`Set`。  
但如果你想真正把这块讲清楚，最好分两层看：

1. **语言层的数据值类型**：JS 到底能表示哪些值
2. **运行时提供的内置结构**：这些值内部是如何组织、存储和访问的
...
- 值本身通常不可再拆成“属性集合”
- 通常按值语义使用
- 除了 `null` 和 `undefined`，访问属性时会发生装箱
- 本身不是“可扩展容器”

#### 2. 对象（Object）

除了原始值以外，几乎所有复杂结构都是对象，包括：
...
虽然严格来说原始值不是我们日常说的“容器型数据结构”，但它们是 JS 一切数据表达的基础。
...
它很重要，因为从“数据结构视角”看，字符串其实是 JS 内置的**只读顺序结构**之一。
...
console.log(obj['name']);   // 下标语法
console.log('name' in obj); // 是否存在（含原型链）
console.log(Object.hasOwn(obj, 'name')); // 是否自有属性
```

#### 7. 时间复杂度的直觉理解

在引擎优化正常时，大多数属性读写可以近似理解为：
...
- 按索引读取：通常近似 `O(1)`
- 尾部 `push` / `pop`：通常近似 `O(1)`
- 头部 `shift` / `unshift`：通常 `O(n)`
- 中间插入删除 `splice`：通常 `O(n)`
- 查找 `indexOf` / `find`：通常 `O(n)`

#### 7. 数组常见角色

数组在面试里经常被当成很多经典结构的承载体：

- 栈：`push` / `pop`
- 队列：`push` / `shift`
- 双端队列：`push` / `pop` / `unshift` / `shift`
...
很多人不会把函数归到“数据结构”里，但在 JS 中，函数本质上也是对象。
...
所以从语言设计上看，函数是 JS 内置对象系统里非常特殊的一种“可调用对象”。
...
**只关心“有没有这个值”，而不关心“这个值对应什么键”的结构**

#### 3. 判重依据

`Set` 使用的是 `SameValueZero` 语义，大致可以理解为：
...
如果一个对象只剩下 `WeakMap` 这类弱引用，没有其他强引用指向它，那么垃圾回收器可以回收它。
...
这一组是很多前端同学容易忽视，但在底层能力、文件处理、网络协议、音视频、WebGL 中非常重要。

#### 1. `ArrayBuffer`

`ArrayBuffer` 表示一块**原始二进制内存区域**。

它本身不关心里面是什么类型的数据，只是一段字节缓冲区。
...
`TypedArray` 不是单个类，而是一组“带类型视图”的数组结构，例如：
...
- 普通数组更灵活
- `TypedArray` 更接近底层内存模型

#### 3. `DataView`

`DataView` 是对 `ArrayBuffer` 的一种更灵活的读写视图。
...
- `DataView` 就是“手动精细控制的二进制读写器”

#### 4. `SharedArrayBuffer`

`SharedArrayBuffer` 是可在多个执行上下文之间共享的底层二进制缓冲区。
...
这个结构更偏底层并发编程，一般业务代码不常直接使用。

---

### 十一、字符串也是一种特殊的顺序结构

很多人把字符串只当“基本类型”，但从数据组织角度，它其实也是非常重要的内置序列结构。
...
严格来说，`Date` 和 `RegExp` 不属于“通用集合结构”，但它们也是 JS 内置对象系统中的特殊数据对象。
...
### 十三、为什么 `Object`、`Map`、`Set` 最值得重点掌握

因为这三个最能代表 JS 中三类核心组织方式：
...
> 日常开发最核心的是 `Object`、`Array`、`Map`、`Set`；涉及对象私有元数据时常用 `WeakMap`；涉及二进制和底层性能时会用到 `ArrayBuffer`、`DataView` 和 `TypedArray`。

---

### 十八、一张表快速对比

| 结构 | 是否有序 | 键/成员类型 | 是否可遍历 | 是否可重复 | 典型用途 |
| --- | --- | --- | --- | --- | --- |
| `Object` | 属性遍历有规则，但不建议把它当严格顺序容器 | 字符串 / `Symbol` 键 | 可以 | 键不能重复 | 实体、配置、JSON |
| `Array` | 有序 | 任意元素 | 可以 | 可以 | 列表、顺序数据 |
| `Map` | 有序（按插入顺序） | 任意键 | 可以 | 键不能重复 | 通用映射、缓存 |
| `Set` | 有序（按插入顺序） | 任意成员 | 可以 | 成员不能重复 | 去重、成员集合 |
| `WeakMap` | 不强调 | 仅对象键 | 不可以 | 键不能重复 | 对象私有元数据 |
| `WeakSet` | 不强调 | 仅对象成员 | 不可以 | 成员不能重复 | 对象标记 |
| `ArrayBuffer` | 字节序列 | 原始字节 | 间接使用 | 可以重复 | 二进制内存 |
| `TypedArray` | 有序 | 固定数值类型 | 可以 | 可以 | 高性能数值处理 |
| `DataView` | 按字节视图 | 面向偏移读写 | 间接使用 | 不适用 | 精细二进制解析 |

---

### 十九、最后的理解框架

你可以把 JS 内置数据结构记成下面这棵树：
...
└─ 对象
   ├─ 普通对象：Object
   ├─ 顺序结构：Array / String（值类型但像序列）
   ├─ 可调用对象：Function
   ├─ 键值映射：Map / WeakMap
   ├─ 唯一值集合：Set / WeakSet
   ├─ 二进制结构：ArrayBuffer / DataView / TypedArray / SharedArrayBuffer
   └─ 特殊数据对象：Date / RegExp
```
...
JS 的内置数据结构并不只是 `Object` 和 `Array`。  
如果从完整视角看，它包含：

- 原始值体系：负责表达最基本的数据单元
- 对象体系：负责表达结构化、可扩展、可共享行为的数据
- 集合体系：`Map` / `Set` / `WeakMap` / `WeakSet`
- 二进制体系：`ArrayBuffer` / `DataView` / `TypedArray`

真正写业务时，最常用的是 `Object`、`Array`、`Map`、`Set`；  
真正拉开理解层次的，是你是否知道为什么要有 `WeakMap`、为什么字符串是不可变序列、以及为什么二进制结构和普通数组完全不是一个层面的东西。