...
因为 Certbot 申请证书时需要占用 80 端口。
原理
Let's Encrypt 验证你拥有这个域名的方式是：
Certbot 在本机启动一个临时 HTTP 服务器，监听 80 端口
Let's Encrypt 服务器访问 http://api.paaawow.com/.well-known/acme-challenge/xxx
如果能访问到 Certbot 返回的验证内容，就证明你控制这个域名，颁发证书
为什么要停 Nginx？
如果 Nginx 容器正在运行并占用 80 端口，Certbot 就无法绑定 80 端口，会报错：
所以需要先停掉 Nginx，让 Certbot 临时使用 80 端口完成验证，拿到证书后再启动 Nginx。
另一种方式（不停 Nginx）：使用 DNS 验证，但需要手动去域名服务商添加 TXT 记录，操作更麻烦。--standalone 方式最简单快捷。




docker-compose.yml中已经配置了extra_hosts让Linux也支持host.docker.internal



配置文件有/ws/路由，但nginx返回404。需要重新加载nginx配置
...
1. ufw (Ubuntu防火墙)
状态：Inactive（未启用）
规则：已添加16006、16007、16008端口，但未生效
2. iptables
状态：运行中
主要规则：
YJ-FIREWALL-INPUT - 云服务商的防火墙链
Docker网络规则（自动管理）
没有自定义规则
3. 云服务器安全组（真正的防火墙）
这是主要的访问控制层
