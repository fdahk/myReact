...
# 包管理
    ...
    3.HEAD状态分离： head
      HEAD指向一个分支引用（refs/heads/JiangTu）
      分支引用再指向一个具体的commit
      这是一个双层指向关系
      原因：
      origin/JiangTu不是本地分支，而是远程跟踪分支
      远程跟踪分支只是一个只读引用
      Git不让你直接在远程跟踪分支上工作
      时间线示例：
...
## 关于部署时的ssh密钥处理方式
easingthemes/ssh-deploy 依赖系统的 OpenSSL libcrypto 库来解析 SSH 密钥，但这个库对密钥格式要求严格（特别是无法识别 OpenSSH 新格式），所以报错 "error in libcrypto"；而 appleboy/scp-action 使用纯 Go 语言的 SSH 实现，原生支持所有密钥格式（RSA、ED25519、OpenSSH 新旧格式），不依赖系统工具，容错性强，所以能正常工作。