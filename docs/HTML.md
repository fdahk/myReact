...
`<ins>` 是 **inserted text（插入文本）标签**，用于表示“这段内容是后来新增的”。  
它通常与 `<del>` 配合使用，用来表达文档修订记录（新增/删除）。
...
1. **语义功能分类**：属于“编辑修订类（edits）标签”  
   - 用来标记内容变更历史（尤其是“新增”）
2. **内容模型分类**：属于 **phrasing content（短语内容）**  
   - 也就是常说的**行内语义标签（inline）**
3. **标签形态分类**：属于**双标签（非自闭合）**  
   - 必须写开始和结束标签：`<ins>...</ins>`
...
- `<del>`：标记被删除的内容（和 `<ins>` 最常一起出现）

按“同属行内语义/phrasing content”来看，常见还有：

- `<span>`：通用行内容器（无特定语义）
- `<em>`：强调（emphasis）
- `<strong>`：强强调（strong importance）
- `<mark>`：高亮标记
- `<small>`：附注/小号文本
- `<s>`：不再准确或不再相关的文本
- `<u>`：需要特别标注的文本
...
`<ins>` 除了全局属性外，还有两个和修订相关的常用属性：

- `cite`：指向修订说明的 URL
- `datetime`：修订时间（建议使用合法日期时间格式）

示例：

```html
<ins cite="https://example.com/changelog" datetime="2026-03-05T10:30:00+08:00">
...
- 浏览器默认通常会给 `<ins>` 加下划线
- 如果只是“视觉下划线”，不要滥用 `<ins>`，可用 CSS（如 `text-decoration`）
- 只有在“语义上确实表示新增内容”时才用 `<ins>`